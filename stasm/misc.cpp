...
cv::Scalar ToCvColor(unsigned color)
{
    CvScalar cvcolor;
        ...
        if (i < shape.rows)
        {
            ...
            else // lines
            {
                ...
                while (j < shape.rows && !PointUsed(shape, j))
                    j++;
                if (j < shape.rows) {
                    // Printing point indexes on image
                    //std::stringstream out;
                    //out << point_idx;
                    //cv::putText(img, out.str(), cv::Point(cvRound(shape(i, IX)), cvRound(shape(i, IY))),
                    //            cv::FONT_HERSHEY_SIMPLEX, 0.25, cv::Scalar(0, 255, 0));

                    if (point_idx > 14) { // the last face contour point
                        int mid_x = cvRound(shape(0, IX) + (shape(15, IX) - shape(0, IX)) / 4);
                        int mid_y = cvRound(shape(15, IY) + (shape(0, IY) - shape(15, IY)) / 4);

                        cv::line(img,
                                 cv::Point(cvRound(shape(i, IX)), cvRound(shape(i, IY))),
                                 cv::Point(mid_x, mid_y),
                                 cvcolor, linewidth);

                        cv::line(img,
                                 cv::Point(mid_x, mid_y),
                                 cv::Point(cvRound(shape(0, IX)), cvRound(shape(0, IY))),
                                 cvcolor, linewidth);
                        break;
                    }

                    if (point_idx == 12) {
                        int mid_x = cvRound(shape(13, IX) + (shape(12, IX) - shape(13, IX)) * 0.75);
                        int mid_y = cvRound(shape(13, IY) + (shape(12, IY) - shape(13, IY)) * 0.25);

                        cv::line(img,
                                 cv::Point(cvRound(shape(12, IX)), cvRound(shape(12, IY))),
                                 cv::Point(mid_x, mid_y),
                                 cvcolor, linewidth);

                        cv::line(img,
                                 cv::Point(mid_x, mid_y),
                                 cv::Point(cvRound(shape(13, IX)), cvRound(shape(13, IY))),
                                 cvcolor, linewidth);
