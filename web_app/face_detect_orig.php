...
$processed_photo_output_path = $processed_photo_output_path. basename( pathinfo($_FILES['uploadedFile']['name'], PATHINFO_FILENAME)) . '.png';
$downloadFileName = 'processed_' . basename( pathinfo($_FILES['uploadedFile']['name'], PATHINFO_FILENAME)) . '.png';
...
if (move_uploaded_file($_FILES['uploadedFile']['tmp_name'], $photo_upload_path)) {
    $command = "LD_LIBRARY_PATH=/usr/lib ./facecut -i $photo_upload_path -o $processed_photo_output_path &> /dev/null";
    exec($command);
